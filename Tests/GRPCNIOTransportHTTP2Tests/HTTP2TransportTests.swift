...
final class HTTP2TransportTests: XCTestCase {
  ...
  private func makeClient(
    ...
    case .tunnel:
      ...
      var config = GRPCTunnelClientTransport.Config.defaults
      ...
      let tunnel = GRPCTunnelClientTransport(takingOwnershipOf: channel, config: config)
      transport = NIOClientTransport(tunnel)
    }
...
extension [HTTP2TransportTests.Transport] {
  static let supported: [HTTP2TransportTests.Transport] = [
    HTTP2TransportTests.Transport(server: .posix, client: .posix),
    HTTP2TransportTests.Transport(server: .posix, client: .tunnel),
  ]

  // TODO: This should be the supported transports:
