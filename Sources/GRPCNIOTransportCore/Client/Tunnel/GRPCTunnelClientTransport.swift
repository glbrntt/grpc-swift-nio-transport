...
public final class GRPCTunnelClientTransport: ClientTransport {
  ...
  private func makeStream(
    ...
    if let override = options.compression {
      compression = self.config.compression.enabledAlgorithms.contains(override) ? override : .none
    } else {
    ...
    do {
      let stream = try await multiplexer.openStream { channel in
        ...
        }  // TODO: .runInitializerIfSet(connected.onCreateHTTP2Stream, on: channel)
      }
